...
use crate::{
    ...
    ipc::notebooklm::NotebookLMEngineStatus,
};

use super::http_client::{HealthzResponse, NotebookLMHttpClient};

const STARTUP_TIMEOUT: Duration = Duration::from_secs(20);
const HEALTH_POLL_INTERVAL: Duration = Duration::from_millis(500);
const CIRCUIT_WINDOW: Duration = Duration::from_secs(5 * 60);
const EXPECTED_ENGINE_VERSION: &str = "0.1.1";
...
struct EngineSupervisorInner {
    ...
    runtime_dir: PathBuf,
    data_dir: PathBuf,
    status: Arc<ParkingMutex<NotebookLMEngineStatus>>,
...
impl EngineSupervisor {
    ...
    pub async fn refresh_status(&self) -> NotebookLMEngineStatus {
        ...
        let status = NotebookLMEngineStatus {
    ...
    pub async fn ensure_started(&self, force: bool) -> Result<NotebookLMEngineStatus, AppError> {
        ...
        if let Ok(health) = self.inner.http_client.healthz().await {
            ...
            if is_notebooklm_engine_service(&health) {
                self.replace_stale_engine(&health).await?;
            }
        }
        ...
        if self.port_in_use() {
            return Err(AppError::new(
                ...
                format!(
                    "端口 {} 已被未知进程占用，无法启动 notebooklm-engine",
                    self.inner.port
                ),
                false,
            )
            ...
            if force {
                ...
                if open_until > now {
                    ...
                    return Err(AppError::new(
                        ...
                        "notebooklm-engine 当前处于熔断冷却期，请稍后重试",
                        true,
                    )
        ...
        loop {
            ...
            if Instant::now() >= deadline {
                ...
                return Err(AppError::new(
                    AppErrorCode::NotebooklmEngineUnavailable,
                    "notebooklm-engine 启动超时，20 秒内未通过健康检查",
                    true,
                ));
            }
    ...
    pub async fn shutdown(&self, force: bool) -> Result<NotebookLMEngineStatus, AppError> {
        ...
        if matches!(
            ...
            let _ = self.inner.http_client.shutdown().await;
        }
    ...
    async fn status_from_health(&self, health: Option<HealthzResponse>) -> NotebookLMEngineStatus {
        let (pid, circuit_breaker_open) = {
            let mut state = self.inner.state.lock().await;
            state.breaker.purge(Instant::now());
            (
    ...
    async fn record_runtime_failure(&self) {
        let mut state = self.inner.state.lock().await;
            ...
            match status {
                ...
                _ => {
                    return Err(AppError::new(
                        AppErrorCode::NotebooklmEngineUnavailable,
                        format!(
                            "缺少 NotebookLM 运行依赖 {module_name}，请检查 requirements.txt 安装状态"
                        ),
                        false,
                    )
                    .with_detail("python", python.clone())
    ...
    fn spawn_child(&self, python: &str) -> Result<Child, AppError> {
        let stdout_log = OpenOptions::new()
            .create(true)
            .append(true)
            .open(self.inner.runtime_dir.join("notebooklm-engine.stdout.log"))?;
        let stderr_log = OpenOptions::new()
            .create(true)
            .append(true)
            .open(self.inner.runtime_dir.join("notebooklm-engine.log"))?;
        ...
        command
            ...
            .env("PYTHONPATH", &pythonpath)
            .env("RASTRO_NOTEBOOKLM_ENGINE_HOST", &self.inner.host)
            .env("RASTRO_NOTEBOOKLM_ENGINE_PORT", self.inner.port.to_string())
            .env(
                "RASTRO_NOTEBOOKLM_DATA_DIR",
        ...
        let child = command.spawn().map_err(|error| {
            AppError::new(
                AppErrorCode::NotebooklmEngineUnavailable,
                format!("启动 notebooklm-engine 失败: {error}"),
                true,
            )
        ...
        fs::write(
            self.inner.runtime_dir.join("notebooklm-engine.pid"),
            child.id().to_string(),
        )?;
        Ok(child)
    }
    ...
    fn write_status(&self, status: NotebookLMEngineStatus) {
        *self.inner.status.lock() = status;
    }

    fn cleanup_runtime_markers(&self) -> Result<(), AppError> {
        let pid_file = self.inner.runtime_dir.join("notebooklm-engine.pid");
        if pid_file.exists() {
    ...
    async fn replace_stale_engine(&self, health: &HealthzResponse) -> Result<(), AppError> {
        let existing_version = health.engine_version.clone().unwrap_or_default();
        let _ = self.inner.http_client.shutdown().await;
        ...
        Err(AppError::new(
            AppErrorCode::NotebooklmEngineUnavailable,
            format!(
                "检测到旧版 notebooklm-engine ({existing_version}) 仍在运行，且无法自动下线，请重启应用后重试"
            ),
            true,
        )
        .with_detail("existingVersion", existing_version)
        .with_detail("expectedVersion", EXPECTED_ENGINE_VERSION))
    }
}
