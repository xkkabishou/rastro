...
2. **参考文献/致谢页检测** (`detect_reference_pages`, `detect_acknowledgement_pages`) -- 自动跳过无需翻译的尾页
...
检测参考文献页（1-based），策略：标题在页面前 20% 则该页及后续跳过，否则仅跳后续页。
...
- `antigravity_translate/core.py` -- 核心翻译逻辑（363 行）
